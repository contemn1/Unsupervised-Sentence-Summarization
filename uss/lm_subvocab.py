...
def prob_next(LMModel, vocab, text, hn=None, subvocab=None, clustermask=None, onscore=False, renorm=False):
    ...
    with torch.no_grad():
        ...
        # size: (seq_len, batch_size=1, vocab_size)
        output = LMModel.proj(output)
...
def prob_next_1step(LMModel, batch_text, hn=None, subvocab=None, clustermask=None, onscore=False, renorm=False, temperature=1):
    ...
    with torch.no_grad():
        ...
        embed = LMModel.embedding(batch_text)
        output, hn = LMModel.lstm(embed, hn)
        # size: (seq_len=1, batch_size=onbeam_size, vocab_size)
        output = LMModel.proj(output)

    output = output / temperature
    # size: (batch_size=onbeam_size, vocab_size)
...
#         return
    ...
    # cluster on the probabilities
    # size: (batch_size=onbeam_size, subvocab_size)
    subprobs = probs[:, subvocab]
    if clustermask is None:
        if renorm:
...
def prob_sent(LMModel, vocab, text, hn=None, subvocab=None, clustermask=None, onscore=False, renorm=False, size_average=False):
    ...
    if subvocab is None:
        with torch.no_grad():
            ...
            # size: (seq_len, batch_size=1, vocab_size)
            output = LMModel.proj(output)
        ll = torch.nn.functional.cross_entropy(output.squeeze(
        )[:-1, :], textid[1:], size_average=size_average, ignore_index=LMModel.padid)
        ll = -ll.item()
        return ll, hn
    ...
    textid_sub = next(LMModel.parameters()).new_tensor([subvocab.numpy().tolist().index(vocab.stoi[w]) for w in text],
                                                       dtype=torch.long)
    subprobs_sent = torch.zeros(
        len(text) - 1, len(subvocab), device=next(LMModel.parameters()).device)
    for i in range(len(text) - 1):
        subprobs, probs, hn = prob_next(
            LMModel, vocab, text[i], hn, subvocab, clustermask, onscore, renorm)
        subprobs_sent[i] = subprobs
    ll = torch.nn.functional.nll_loss(torch.log(
        subprobs_sent), textid_sub[1:], size_average=size_average, ignore_index=LMModel.padid)
    ll = -ll.item()
    return ll, hn
...
def clmk_nn(embedmatrix, subvocab, normalized=True):
    ...
    if normalized:
        sim_table = sim_table / \
            torch.ger(submatrix.norm(2, 1), embedmatrix.norm(2, 1))
...
def clmk_cn(embedmatrix, subvocab, simthre=0.6, normalized=True):
    ...
    # set difference
    clustermask = (clustermask ^ subvocabmask) & clustermask
    for i in range(len(subvocab)):
        # add back the current word in the sub-vocabulary
        clustermask[i][subvocab[i]] = 1
    if torch.__version__ >= '1.2.0':
        clustermask = clustermask.bool()
    return clustermask
